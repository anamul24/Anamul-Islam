...
export default function Hero() {
  const [heroData, setHeroData] = useState({ roles: DEFAULT_ROLES, name: 'Anamul Islam', scrollHint: 'Scroll to explore work' });

  useEffect(() => {
    fetch('/api/admin/data?section=hero')
      .then(r => r.json())
      .then(d => { if (d.roles?.length) setHeroData(d); })
      .catch(() => {});
  }, []);

  const typedText = useTypingAnimation(heroData.roles);
