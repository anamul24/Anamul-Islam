...
import { Cpu, Terminal, Database, Palette, Globe, Server, Wifi, Shield, Network } from 'lucide-react';
...
export default function TechMarquee() {
  ...
  useEffect(() => {
    fetch('/api/admin/data?section=skills')
      ...
      .then(d => {
        if (Array.isArray(d)) {
          setTechStack(d.map(s => ({ ...s, icon: ICON_MAP[s.category] || <Terminal size={18} /> })));
        }
      })
      .catch(() => {});
  }, []);
